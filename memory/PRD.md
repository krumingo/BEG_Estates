...
**Last updated:** 2026-05-10 (iteration 26)
**Status:** Iteration 26 — R.6 Tab-based management dashboard (v2.0.0)

## Iterations
- **v2.0.0 (2026-05-10)** — **R.6 — Tab-based management dashboard (major)**: пълно преработване на dashboard-а в management tool за продажби, финанси, клиенти, вноски, календар и непродаден инвентар. Backend: нов `services/dashboard_aggregations.py` (~580 lines) — `build_dashboard()` използва `deals.items[].agreed_price` като основен източник за продадена стойност, `deals.bank_stages + own_stages` за плащания (paid/expected/overdue/deposits с правилно detection чрез is_paid/is_deposit/label/milestone_type), legacy `payments/payment_installments` само като fallback, multi-property deal split по items, project filter обхваща само deal items в обхвата. `routes/dashboard.py` опростен в thin wrapper с 8 query params (project_id, building_id, property_type, status, client_id, period, only_overdue, only_available). Финансова видимост: super_admin/admin/**accounting**. Нови response блокове: overview, finance, sales_pipeline, by_type, by_floor, by_building, clients_summary, money_calendar, unsold_inventory, action_items. Запазени legacy keys: cash, sales, calendar, top_clients, recent_sales, recent_inquiries, alerts. Frontend: `AdminDashboard.jsx` напълно преработен (tab-based, 152 lines), нов `DashboardFilters.jsx` (163 lines, 7 филтъра), нов `DashboardTabs.jsx` (700 lines, 6 tab-а): Обзор (4 stat cards + pipeline funnel + action items + recent mini), Продажби (recharts bar charts + 3 breakdown tables + clickable type rows), Финанси (6 stat cards + expected 7d/30d/90d + 2 month bar charts + payment mode pie chart), Календар (3 period cards + bar chart + upcoming/overdue tables), Клиенти (8-колонна таблица със status badges), Непродадени (4 stat cards + risk-flagged table). Тествано 100% pass от testing_agent_v3_fork (iteration_14.json: 38/38 cases). Стария R.5 dashboard (CashCards, SalesCards и т.н.) остават неизползвани файлове в /components/admin/.
