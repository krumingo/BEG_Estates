...
**Last updated:** 2026-05-11 (iteration 33)
**Status:** Iteration 33 — R.9 Market Logic Fix + Sellable Breakdown (v2.3.0)

## Iterations
- **v2.3.0 (2026-05-11)** — **R.9 — Dashboard Market Logic Fix**: (1) Backend `dashboard_aggregations.py` обогатен с правилния split: `market_available_*` (=available+reserved_zero+reserved_paid_deposit) и `non_sale_*` (=compensation+hidden+unavailable). Запазен е `not_sold_*` за обратна съвместимост — вече само като alias за `market_available + non_sale`. Добавени са в overview: `market_available_area`, `market_available_value_net/with_vat`, `non_sale_area`, `non_sale_value_visual_only_net/with_vat`. Документирани invariants. (2) Frontend `OverviewTab`: 4-тата hero карта „Непродадени" преименувана на „На пазара" (data-testid `overview-card-on-market`), показваща `market_available_count` (=30 за реалния проект, не 47). Добавена нова секция „Продаваем потенциал" с 4 status cells (Свободни / Без капаро / С капаро / Общо потенциал), видима само за финансови роли. (3) `.gitignore` презаписан със чисто 90-редово (=78 ефективни реда без коментари/blank) съдържание. Backend пиtest: 6/6 PASS. Reconciliation спрямо реалните 52 имота: **52=5+30+17+0** count, **1933.85=415.95+907.97+609.93** area (exact match).
- **v2.2.0 (2026-05-11)** — **R.8 — Dashboard Overview Area/Value Enrichment**: Backend `services/dashboard_aggregations.py` обогатен с canonical helpers `prop_area(p)` (fallback chain `area_total → total_area → saleable_area → built_up_area → area → raw_area → area_pure`), `prop_value(p)` (`list_price → base_price → price_per_sqm * area`), `pct(part, whole)`. `overview` блокът сега носи нови area полета: `total_area / total_rzp_area`, `sold_area / sold_area_percent`, `compensation_area / compensation_area_percent`, `not_sold_area / not_sold_area_percent`, `available_area`, `reserved_zero_area`, `reserved_deposit_area`, `hidden_area`, `unavailable_area`, `hidden_unavailable_area`. Финансови полета (само за финансови роли): `total_market_value_with_vat`, `sold_value_with_vat`, `not_sold_value_with_vat`, `compensation_value_visual_only_with_vat`, `hidden_unavailable_value_visual_only_with_vat`, `reserved_zero_value_with_vat`, `reserved_deposit_value_with_vat`. Sold value от `agreed_price` в deals (fallback на `prop_value`); всичко друго от `prop_value`. `by_type / by_floor / by_building` използват shared `_new_breakdown_rec` + `_add_to_breakdown` + `_finalize_breakdown` helpers — всеки ред има per-status count + area + value_net + value_with_vat. Frontend `OverviewTab`: 4 hero карти (Общо имоти / Продадени / Обезщетение / Непродадени) с multi-line JSX `sub` (count → м² → %от РЗП → стойност). Status section: 6 cells с `count + value + area` (нови data-testid суфикси `-value` и `-area`); label преименуван на „Скрити / неактивни". Тествано 100% pass (iteration_18.json: 30/30 features, 17/17 backend pytest). НЕ е пипан funnel bug-ът при „С капаро" (запазен за отделна задача), НЕ е пипана Construction Cashflow логика, НЕ са пипани permissions.
- **v2.1.3 (2026-05-11)** — **R.7.2 Cleanup Retry**: (1) `.gitignore` повторно почистен — премахнати 3 reintroducednatи дирти реда (`50.pack`, `52.pack`, `-e `) и един дубликат на „# Environment and credential files" блок. Финал: 90 реда, без `-e`, без `default-development`, без конкретни `.pack` имена. (2) `test_reports/iteration_1.json` и `iteration_2.json` (които бяха пропуснати в v2.1.2) сега са scrub-нати — всички `Admin123!`, `Sales123!`, `BegEstates2026!*` и email/password комбинации заменени с `[REDACTED]`. Тотално 16 redacted test_reports файла в repo-то. ZIP: 569K, без `__pycache__`, без `.pyc`, без webpack `.pack` файлове, без credential leaks. R.7 dashboard / cashflow / permission логиката не е пипана.
- **v2.1.2 (2026-05-11)** — **Final R.7 Cleanup**: (1) `.gitignore` повторно почистен — премахнат re-introducnatят `50.pack` ред и единичен `-e` ред (582 → 90 реда, окончателно). (2) Redacted test credentials в `test_reports/iteration_3.json` до `iteration_17.json` — заменени всички `*@begestates.bg / BegEstates2026!*` низове с `[REDACTED]` (14 файла). Реалните credentials остават само в `memory/test_credentials.md` (вече в .gitignore). Working tree почистен от `__pycache__`/`.pyc`. ZIP audit: no `__pycache__`, no `.pyc`, no `.pack`, no credential leaks.
- **v2.1.1 (2026-05-11)** — **R.7 Cleanup + Permission Fix**: (1) Construction Cashflow таб остава видим за всички финансови роли (super_admin, admin, accounting), но редакцията е заключена само за super_admin. Frontend използва `useIsSuperAdmin()` от `lib/auth.jsx` → `canEdit` boolean → `<fieldset disabled>` за всички inputs + save бутон скрит + `construction-readonly-notice` със текст „Само super_admin може да редактира строителната прогноза." за admin/accounting. (2) Почистен `.gitignore` от 582 → 90 реда (премахнати ~70 дублиращи се блока „# Environment and credential files" и `-e` редове, премахнати конкретни `*.pack` имена в полза на общия `*.pack` pattern). (3) Изтрити всички `__pycache__/` и `*.pyc` файлове от working tree. Тествано 100% pass (iteration_17.json: 28/28 features).
- **v2.1.0 (2026-05-11)** — **R.7 — Construction Cashflow Forecast**: Нов седми таб „Строителство" в Tab-based Admin Dashboard, видим само за финансови роли (super_admin, admin, accounting), скрит за sales. Backend: `Project.construction_cashflow_settings` (total_rzp_area, rough_cost_per_sqm, full_cost_per_sqm, rough_frontload_months/percent, cash_opening_balance, minimum_cash_reserve, reserve_percent, rough_start_date, forecast_months, notes) + нов сервиз `services/construction_cashflow.py` (frontload разпределение на разходите, месечен cashflow, баланс vs резерв, статус ok/below_reserve/deficit, аларми critical/warning/info, recommended_credit_buffer). Интегриран в `dashboard_aggregations.build_construction_cashflow()` и достъпен само когато is_finance_visible=True и project_id е подаден. Frontend: `ConstructionCashflowTab.jsx` (форма с настройки → PUT /admin/projects/{id}, KPI карти РЗП/Rough/Full/Revenue-until-Act14/Cash/Deficit/Credit-buffer, ComposedChart с приходи vs разходи vs баланс vs резерв, месечна таблица с цветни статус badges) интегриран в AdminDashboard.jsx с `reloadKey` за refresh след save. Тествано 100% pass от testing_agent_v3_fork (iteration_16.json: 14/14 features, 7/7 backend tests).
- **v2.0.1 (2026-05-10)** — **R.6.1 — Dashboard Visual + Counting Fix**: Backend overview блок разширен с 13 нови counting полета (total_properties, sold_count, **not_sold_count**, available_count, reserved_count, reserved_zero_count, reserved_deposit_count, market_available_count, compensation_count, hidden_count, unavailable_count, non_sale_count, other_count) + 5 нови finance полета (reserved_value_with_vat, sellable_potential_with_vat, compensation_value_visual_only_with_vat и т.н.) + `count_reconciliation_ok` flag. Реши проблема с объркването: 18 апартамента = 5 sold + 6 available + 7 compensation, not_sold=13. Frontend OverviewTab пренаписан: HERO row с 3 големи карти (Общо имоти dark, Продадени зелена, Непродадени амбер, value text-5xl) + новата `Статус на инвентара` секция с 6 цветни StatusCell-а (свободни emerald, резерв.без капаро amber, с капаро orange, продадени slate, обезщетение violet, скрити stone). Визуални upgrade-и: rounded-2xl cards с gradient backgrounds (from-emerald-50, from-slate-900 за dark и т.н.), shadow-sm hover:shadow-md, charts h-80 (преди h-64), tables text-base (преди text-sm), tab triggers text-sm px-4 py-2. Recharts с цветни Bars (зелени Свободни, амбер Резервирани, тъмни Продадени) вместо slate-only. Тествано 100% pass от testing_agent_v3_fork (iteration_15.json: 21/21 cases + 9/9 backend pytest).
- **v2.0.0 (2026-05-10)** — R.6 Tab-based management dashboard
- **v1.6.0 (2026-05-09)** — R.5 ЧАСТ 4 ФИНАЛНА — Calendar + TopClients + Alerts
